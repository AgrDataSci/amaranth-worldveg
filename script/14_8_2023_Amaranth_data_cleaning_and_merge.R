...
# get first word from "uniquenames"
first_word<- c()
for (i in seq_along(uniquenames)){
  first_word[i] <- strsplit(uniquenames, "_")[[i]][1]}
...
dat = lapply(dat, function(x){
  ...
  names(x)[names(x) == "transplantation"] = "transplanting"
  names(x)[names(x) == "performanceagronomique1_"] = "firstharvest_"
  names(x)[names(x) == "premiererecolte_"] = "firstharvest_"
  names(x)[names(x) == "performanceagronomique2"] = "secondharvest"
  names(x)[names(x) == "deuxiemerecolte"] = "secondharvest"
  names(x)[names(x) == "performanceagronomique3"] = "thirdharvest"
  names(x)[names(x) == "troisiemerecolte"] = "thirdharvest"
  names(x)[names(x) == "quatriemerecolte"] = "fourthharvest"
  names(x)[names(x) == "floraison"] = "reproductive"
  names(x)[names(x) == "socio_economic"] = "socioeconomic"
  names(x) = gsub("overallchar2", "overall", names(x))
  x
})
############## However, does this code necessary?...
...
write.table(dat,file="data/Amaranth_tricot_newversion.csv",  append=FALSE, sep= "," , row.names = FALSE, col.names=TRUE)
